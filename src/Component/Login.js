...
const Login = () => {
  ...
  //   let data = secureLocalStorage.getItem("n_otp");
  //   if (data != null) {
  //     return data
  //   } else {
  //     return " "
  ...
  let otpvalue = secureLocalStorage.getItem("n_otp");
  let _id = secureLocalStorage.getItem("n_id");
  let token = secureLocalStorage.getItem("n_token");
  let mobile = secureLocalStorage.getItem("n_mobile_no");
  //inputvalue
  ...
  let formHandel = (event) => {
    ...
    let mmobile = /^\d{10}$|^\d{3}[-\s]\d{3}[-\s]\d{4}$/
    let eemail = /^[a-zA-Z0-9._%+-]+@gmail\.com$/i;
    if (eemail.test(inputval.email)) {
      setemailmsg("")
    } else if (!eemail.test(inputval.email)) {
      setemailmsg("Wrong-Email")
    }
    if (mmobile.test(inputval.mobile_no)) {
      setnumbermsg("")
      axios.post(`${baseurl[0].apiurl}signup`, inputval).then((res) => {
        console.log(res)
        if (res.data.result) {
          secureLocalStorage.setItem("n_id", res.data._id);
          secureLocalStorage.setItem("n_token", res.data.token);
        ...
        toast.success("All Redy Signup This Number")
      })

    } else if (!mmobile.test(inputval.mobile_no)) {
      setnumbermsg("Wrong-Number")
    }
  }
  ...
  let Loginform = (event) => {
    ...
    let mmobile = /^\d{10}$|^\d{3}[-\s]\d{3}[-\s]\d{4}$/
    if (mmobile.test(loginvalue)) {
      let data = {
        "mobile_no": loginvalue
      }
      setnumbermsg("")
      axios.post(`${baseurl[0].apiurl}login`, data).then((res) => {
        console.log(res)
        localStorage.setItem("localdata", JSON.stringify(res.data.data))
        if (res.data.result) {

          localStorage.setItem("ssstoken", res.data.data.token)
          secureLocalStorage.setItem("n_id", res.data.data._id);
          secureLocalStorage.setItem("n_token", res.data.data.token);
          secureLocalStorage.setItem("n_mobile_no", res.data.data.mobile_no);
          secureLocalStorage.setItem("n_otp", res.data.data.otp);
    ...
    } else if (!mmobile.test(loginvalue)) {
      setnumbermsg("Wrong-Number")
    }
  ...
  let verifyotp = (e) => {
    ...
    axios.post(`${baseurl[0].apiurl}verify_otp`,
      items,
      {
        headers: {
          "token": token,
  ...
  // recent otp  timer 
  let [timestatus, settimestatus] = useState(false)
  ...
  let resendotp = (e) => {
    ...
    axios.post(`${baseurl[0].apiurl}resend_otp`, items).then((res) => {
  ...
  useEffect(() => {
    secureLocalStorage.setItem("n_otp", otpvalue);
    console.log(otpvalue)
  }, [otpvalue])
  ...
  return (
    <div>
      ...
      <div >
        ...
        {/* Modal */}
        <div style={{}} className="modal fade offcanvas-modal" id="exampleModal">
                ...
                <div className="login-register-wrapper">
                  ...
                  <div className="tab-content">
                    <div id="lg1" className="tab-pane show active">
                      <div className="login-form-container" style={{ backgroundColor: "#a3c098" }}>
                        <div className=" login-register-form ">

                          {otpstatus == false ? (

                            <form action="#" method="post">
                              <label style={{ color: "red", fontSize: "10px" }} >{numbermsg}</label>
                              <input type="number" name="mobile_no" onChange={(e) => setloginvalue(e.target.value)} placeholder="Mobile-Number" />
                              ...
                              <div className="button-box">
                                ...
                                <div className='text-center'>
                                  <button onClick={Loginform} className="btn btn-dark">
                                    SEND-OTP
                                  </button>
                            ...
                            <form action="#" method="post">
                              <div className='text-center'>
                                <img width={"30%"} src="assets/images/icon/logo.png" alt="" />
                              </div>
                              <div className='text-center'>
                                {/* <span className='shadow p-2'>Otp - {otpvalue}</span> */}
                              </div>
                              <div className='text-center'>
                                <p style={{ color: "green", padding: "15px" }}>Please Enter Your Otp</p>
                              </div>
                              <div className='row justify-content-center'>
                                <div style={{ width: "60%" }}>
                                  <label style={{ color: "red", fontSize: "10px" }} ></label>
                                  <OtpInput
                                    value={otp}
                                    onChange={setOtp}
                                    numInputs={4}
                              ...
                              <div className='text-center'>
                                {timestatus == false ? (<p>{count}</p>) :
                                  (
                                    <a onClick={resendotp} style={{ color: "blue", cursor: "pointer", marginBottom: "5px" }}>
                                      Resend-Otp
                                    </a>
                                  )}
                              </div>
                    ...
                    <div id="lg2" className="tab-pane">
                      <div className="login-form-container" style={{ backgroundColor: "#a3c098" }}>
                        <div className="login-register-form">
                          <form action="#" method="post" onSubmit={formHandel}>
                            {/* <input type="text" name="user-name" placeholder="Username" /> */}
                            <input required type="text" name="full_name" value={inputval.full_name} onChange={inputvalue} placeholder="Full Name" />
                            {/* <input type="text" name="last-name" placeholder="Last Name" /> */}
                            {/* <input type="password" name="user-password" placeholder="Password" /> */}
                            <label style={{ color: "red", fontSize: "10px" }} >{emailmsg}</label>
                            <input required name="email" value={inputval.email} onChange={inputvalue} placeholder="Email" type="email" />
                            <label style={{ color: "red", fontSize: "10px" }} >{numbermsg}</label>
                            <input required name="mobile_no" value={inputval.mobile_no} maxLength={"10"} onChange={inputvalue} placeholder="Mobile-Number" type="number" />
                            <div className="button-box">
